...
// configMACInfo is the HKDF info string for the config-MAC key (design D2.1).
...
// wrap entry) was edited under an intact manifest store (design D2.3, P0-3,
// T-A2-1) — OR when a device that has previously verified a valid ConfigTag for
// this vault is now served a config with no tag at all (a strip; design D2.4,
// Condition 6). The check runs only AFTER a successful unwrap, so a wrong
...
// D2.1): configMACKey = HKDF-SHA256(masterKey, "seavault-config-mac-v1").
...
// over (design D2.2): the JSON of the config with ConfigTag zeroed, in struct
...
// under the master-derived config-MAC key (design D2.2).
...
// master key, in constant time (design D2.3). A config with no tag returns false
...
// checkConfigIntegrity is Open's post-unwrap config gate (design §2, §5). With a
...
// (ErrConfigTampered, Condition 6); a fresh or genuinely-legacy device (no
...
// Open (D5.4), it only drops rollback protection and records a note.
func (v *Vault) checkConfigIntegrity(opts OpenOptions) error {
	...
	if existed && anc.HasTag {
		...
		// strip, not a legacy vault. Hard refuse (design D2.4, Condition 6).
		return ErrConfigTampered
	}
...
// ConfigTag has already verified (design D5.3/D3.6, §5, T-A2-2): it compares the
...
//     --accept-rollback it clears the anchor and re-TOFUs (Condition 8); otherwise
...
// never blocks Open (D5.4) — it drops rollback protection for the open and notes it.
	...
	if existed {
		...
		case epoch < anc.FormatEpoch:
			if opts.AcceptRollback {
				...
				// re-TOFUs (Condition 8), then re-anchor at the restored epoch below.
...
// single low-level config writer the ratchet uses; slice 4's rewriteConfig builds
// on it.
...
// EnsureConfigMAC is the config-MAC ratchet (design D2.4, Condition 6): on the
...
// seal-format's job, D1.4). It is a no-op (returns false) on a vault that already
// carries a tag, so repeated write-capable opens ratchet exactly once. In this
// slice it is the ONLY writer of the tag; rotation/rewriteConfig lands in slice 4.
//
// The write ordering is config-first, then anchor (design D3.5): vault.json is
...
// already durable — it only drops rollback protection and records a note (D5.4).
