...
// . A 0.15 peer (or any legacy/other-OS client) has no reserved-segment
...
// .seavault, case-insensitively). the design promises such an EXISTING file
...
// same helper the portable-name tests use), which stores real chunks under
...
func TestF3LegacyReservedSegmentFileStaysReachable(t *testing.T) {
	...
	// the existing reserved-segment file is LISTED (not silently hidden).
	list, err := v.List()
	if err != nil {
		t.Fatal(err)
	}
	if !sliceContains(list, reserved) {
		t.Fatalf("existing reserved-segment file must be listed; List = %v", list)
	}

	// it READS back byte-for-byte through WriteFileTo.
	var buf bytes.Buffer
	if err := v.WriteFileTo(reserved, &buf); err != nil {
		t.Fatalf("reading an existing reserved-segment file must work: %v", err)
	}
	...
	if err := v.GetPath(reserved, getOut); err != nil {
		t.Fatalf("getting an existing reserved-segment file must work: %v", err)
	}
	if got, _ := os.ReadFile(getOut); !bytes.Equal(got, payload) {
		t.Fatalf("got content = %q, want %q", got, payload)
	}

	// it EXPORTS (not silently dropped from the export set).
	expDir := t.TempDir()
	res, err := v.ExportPath(context.Background(), "content/", expDir, ExportOptions{})
	if err != nil {
		t.Fatalf("export must not fail: %v", err)
	}
	if !exportWroteFileWithContent(t, expDir, payload) {
		t.Fatalf("the reserved-segment file must be exported; result = %#v", res.Entries)
	}

	// it OVERWRITES in place through PutReader on the existing key.
	...
	if _, err := v.PutReader(bytes.NewReader(newPayload), reserved, int64(len(newPayload)), 0o600, time.Now()); err != nil {
		t.Fatalf("overwriting an existing reserved-segment file must work: %v", err)
	}
	buf.Reset()
	if err := v.WriteFileTo(reserved, &buf); err != nil {
		t.Fatal(err)
	...
	if err := v.Remove(reserved); err != nil {
		t.Fatalf("deleting an existing reserved-segment file must work: %v", err)
	}
	list, err = v.List()
	if err != nil {
		t.Fatal(err)
	...
	//  STILL holds: directly CREATING a NEW reserved-segment virtual path is
	// refused (the create gate is not weakened by the fix).
	for _, p := range []string{"content/SeaVaultData/other.txt", "SeaVaultData/x.txt", ".seavault/x.txt", "sub/SeaVaultData/x.txt"} {
		if _, err := v.PutReader(strings.NewReader("x"), p, 1, 0o600, time.Now()); err == nil {
			t.Fatalf("creating a NEW reserved-segment path %q must still be refused", p)
		}
	}
}
...
// become RECLAIMABLE by gc once removed. A advisory warning must also reach
// the PutReport (never a silent import).
//
// Regression: A1 routed reserved-name segments through the same
...
// a put line that reported "1 new" success. This differs from the
...
func TestConditionsF1DirectoryPutReservedSegmentReachableAndReclaimable(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("directory put must succeed (import as plain content, never refuse): %v", err)
	}

	// the import is NOT silent — a warning reaches the operator-facing report.
	warned := false
	...
	if !warned {
		t.Fatalf("the metadata-dir warning must reach the PutReport; Warnings=%#v", rep.Warnings)
	}
	...
	if !ok {
		t.Fatalf("the foreign file must be indexed at %q; index keys=%v", reserved, indexKeys(idx))
	}
	if len(rec.Chunks) == 0 {
		t.Fatalf("the imported file must reference at least one chunk; rec=%#v", rec)
	}
	chunkID := rec.Chunks[0].ID

	//  reachability: the reserved-segment file is LISTED (never silently hidden).
	list, err := v.List()
	if err != nil {
		t.Fatal(err)
	}
	if !sliceContains(list, reserved) {
		t.Fatalf("the directory-put reserved-segment file must be listed; List=%v", list)
	}

	//  reachability: it READS back byte-for-byte.
	var buf bytes.Buffer
	if err := v.WriteFileTo(reserved, &buf); err != nil {
		t.Fatalf("reading the imported reserved-segment file must work: %v", err)
	}
	if !bytes.Equal(buf.Bytes(), foreignPayload) {
		t.Fatalf("read content = %q, want %q", buf.Bytes(), foreignPayload)
	}

	//  reachability: it GETS to disk.
	getOut := filepath.Join(t.TempDir(), "got.txt")
	if err := v.GetPath(reserved, getOut); err != nil {
		t.Fatalf("getting the imported reserved-segment file must work: %v", err)
	}
	if got, _ := os.ReadFile(getOut); !bytes.Equal(got, foreignPayload) {
		t.Fatalf("got content = %q, want %q", got, foreignPayload)
	}

	//  reachability: the whole subtree EXPORTS, including the foreign file.
	expDir := t.TempDir()
	...
	if !exportWroteFileWithContent(t, expDir, foreignPayload) {
		t.Fatalf("the imported reserved-segment file must be exported, not silently dropped")
	}
	...
	if sliceContains(planBefore.Candidates, chunkID) {
		t.Fatalf("a reachable file's chunk must not be a gc candidate; Candidates=%v", planBefore.Candidates)
	}

	//  reachability: it DELETES.
	if err := v.Remove(reserved); err != nil {
		t.Fatalf("deleting the imported reserved-segment file must work: %v", err)
	}
	list, err = v.List()
	if err != nil {
		t.Fatal(err)
	...
	if !sliceContains(planAfter.Candidates, chunkID) {
		t.Fatalf("after removal the freed chunk must be reclaimable by gc; Candidates=%v", planAfter.Candidates)
	}
}
