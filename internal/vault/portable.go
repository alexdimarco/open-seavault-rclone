...
// checkout even though POSIX would accept it (design D7.1, P1-10). ':' is the
...
// (design D7.1). The check is case-insensitive.
...
// name (design D7.1). The stem is the portion before the first '.', so
...
// this vault and later restored on Windows as well as POSIX (design D7.1). It
...
// form ValidatePortableName's export counterpart (D7.2) would write.
//
// It is a POLICY gate for NEW virtual paths only (D7.1, backlog B6): the callers
...
// nearest portable form (design D7.2): every illegal or control character
