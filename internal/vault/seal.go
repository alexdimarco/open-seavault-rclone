...
// . The shipped 0.16/0.15 client fences on the raw Version
...
// after unseal-format: every A2 member is additive on a Version-2
// config so a 0.16 peer keeps opening it.
const graceVersion = 2

// unsealedMinReader is the MinReader an unsealed vault carries:
...
// would hand them a vault they cannot read. There is
...
// formatTooNew reports the forward-compatibility fence (
// ): a vault whose MinReader exceeds the reader's supported format level is
...
// password entry A2 writes uses the bare AAD (the versioned-AAD write leg
...
// SealFormat is the operator's explicit end of the grace release (
// ): in ONE rewriteConfig transaction it bumps Version 2->3 AND raises
...
func (v *Vault) SealFormat() error {
	return v.rewriteConfig(func(c *VaultConfig) error {
		...
		// seal retires 0.16 access, so the legacy
...
// UnsealFormat reverses SealFormat: in one
...
func (v *Vault) UnsealFormat() error {
	return v.rewriteConfig(func(c *VaultConfig) error {
		...
		// 0.16 peer can unwrap again after unseal: seal cleared
		// it. The entry's own KDF/nonce/CT form a consistent triple.
		if c.WrappedKeys == "" {
