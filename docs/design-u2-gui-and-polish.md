...
STATUS: **BUILT + REVIEWED + FIX TRANCHE APPLIED** (v0.19). The 9 conditions of the pre-code review (`docs/review-u2-predesign.md`, GO_WITH_CONDITIONS; 53 judged / 44 refuted / 0 blockers) are applied below and in §8. Shipped across slices W (vault word core + device-local labels + THIRD_PARTY_NOTICES), G1 (four-destination restructure, Welcome-back view, advanced toggle, plain messages), G2 (recovery UX: words, printable card, labels, last-key revoke, GUI accept-rollback, runtimes on demand), C1 (command registry, synopses, group help, double-dash), and C2 (Type III sweep + docs + final verification).

**Reviewed:** adversarial (`docs/review-u2-adversarial.md`, 13 confirmed / 0 refuted) and friction (`docs/review-u2-friction.md`, 6 Type II / 22 Type III — HIGH_FRICTION_NOT_SHIPPABLE). **Fix tranche** on `feature/u2-gui-restructure`, all findings red-first with no pre-U2 test edited: `22ef5fe` (F-A recovery/label core: word/base32 discriminator, label-store 0600, label GC, providers comment), `9f520e6` (F-B CLI: last-key revoke gate, recovery labels, `--help` exit-code contract, `--save` card, non-interactive refusal), `4949221` (F-C GUI: no-store on phrase, label GC, panel renames, Welcome-back redeem), and the F-D docs/final commit (README reconciled to shipped behaviour; addenda; final verification). All 13 adversarial findings and all 6 friction Type II items are **FIXED**; the 22 Type III rows are tracked backlog with the doc/copy subset applied (see the review addenda). The §5 test matrix is green on the unfiltered `go test -race -count=1 ./...` on Linux with clean windows/amd64 and darwin/arm64 cross-builds and a passing `scripts/smoke-test.sh`.
